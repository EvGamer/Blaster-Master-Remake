...
void World::addShot(float x, float y, float speedX, float speedY,
                    shotType *wpn) {
  shotNode *buf;
  buf = new shotNode;
...
void World::draw(int cx, int cy, unsigned char a_num) {
  ...
  drawSprite(&m_texture, cx, cy, cx + 1, cy + 1, tx + b, ty + b,
             tx + m_size - b, ty + m_size - b);
};
...
void World::setDefault(unsigned first, unsigned last) {
  for (unsigned i = first - 1; i < last; i++)
    set(i, (i) % m_lineLenght, (i) / m_lineLenght, false);
}
...
void World::drawLevel(float scrX, float scrY) {
  ...
  while (cur != NULL) {
    ...
    y0 = cur->y;
    if (!cur->hit)
      cur->flyAnim->draw(dir, x0, y0, x1, y0 + 1);
    else
      cur->burstAnim->draw(1, x2, y0, x2 + 1, y0 + 1);
    cur = cur->next;
  }
}
